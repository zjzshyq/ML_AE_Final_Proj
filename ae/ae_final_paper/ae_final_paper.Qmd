...
The primary hypothesis of our study is that AI-generated artwork significantly influences user preferences and rankings within the online art community. We expect that certain factors, such as the content of AI-generated images, their novelty, and the interaction between users and AI algorithms, will play a crucial role in shaping these preferences. Additionally, we propose a secondary hypothesis that user demographics, art genre preferences, and previous exposure to AI-generated artwork will also impact their rankings and preferences.
...
```{r echo=FALSE}
pixiv <- read.csv('pixiv_tops_lm.csv',header=TRUE, sep=",")
...
## Samples
We collected the data spanning from October 31, 2022, to May 15, 2023 from the top list of AI-generated and man-made image pages. After de-duplicating same image pages which may appear in top with differnet and different day, we gathered the samples:
...
In our analysis, we utilized the R programming language to fit a logit model to the data. Specifically, we employed the logit function，polr [^1], which is suitable for estimating ordered choice models.
...
The Lipsitz test is specifically designed for ordered choice models and assesses the proportional odds assumption. This assumption states that the relationship between the predictors and the response variable is constant across all levels of the response variable. Violation of this assumption may indicate the need for a different modeling approach. The Lipsitz test calculates a chi-square statistic to evaluate the fit of the model to the proportional odds assumption.

The Pulkstenis-Robinson test is another test for assessing the proportional odds assumption in ordered choice models. It examines the parallelism of the cumulative logits across response categories. A non-significant result suggests that the model satisfies the proportional odds assumption.
...
# Findings
## Estimation
To Compare the results of the two models using an academic perspective, we observe both similarities and differences in the qualitative analysis of the variables' effects on the tier outcome in the ordered choice model of probit using ologit.reg algorithm[^8].
...
To analyze the Marginal Effects of the AI-generated artworks model, we examine the output of the code provided. The marginal effects represent the change in the probability of each outcome category based on a unit change in the corresponding independent variable, while holding other variables constant. We choose margins.oglm[^9] to imply marginal effects.
...
[^1]: Please refer to the Appendix of the paper for the relevant code pertaining to the polr algorithm. for logtic model.
...
[^5]: Please refer to the Appendix of the paper for the relevant code pertaining to the clm algorithm.
...
[^8]: Please refer to the Appendix of the paper for the relevant code pertaining to the ologit.reg algorithm.
...
[^2]: Please refer to the Appendix of the paper for the relevant code pertaining to the lipsitz and Lipsitz tests.
...
### pulkrob tests
[^3]: Please refer to the Appendix of the paper for the relevant code pertaining to the  pulkrob tests.
...
[^4]: Please refer to the Appendix of the paper for the relevant code pertaining to the Brant's test.
...
[^6]: Please refer to the Appendix of the paper for the relevant code pertaining to the pR2 test.
...
[^8]: Please refer to the Appendix of the paper for the relevant code pertaining to the anova which is a example of variables selection. 
...
[^9]: Please refer to the Appendix of the paper for the relevant code pertaining to the margins.oglmx algorithm for marginal effects.
