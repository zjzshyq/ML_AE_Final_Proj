...
In our analysis using the R programming language, fitting a logit model firstly to the data. The model equation is as follows:
...
```{r pulkrob, echo=FALSE, warning=FALSE}
...
```{r pulkrob_ai_fixed, echo=FALSE, warning=FALSE}
logit_ai <- polr(tier~like_rate+mark_rate
                  +is_comic+is_Genshin+is_Honkai
                  +comments+top_cnt+date_diff_day
                  , data=ai)
...
```{r brant_ai, message=FALSE, warning=FALSE}
brant(logit_ai)
```
...
- Logit model in Ordered choice model 
